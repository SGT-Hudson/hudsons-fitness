...
`pnpm lint`, `pnpm build`, and `pnpm test` must pass before merge. This is now
...
└── functions/
    ...
    └── recipe-photo-reap/index.ts          # 0 5 * * 0  UTC (R-36b, NOT YET DEPLOYED)
```
...
**and** `--import-map supabase/functions/deno.json` (`config.toml` exists but is
minimal — Tier-3-local-DB only — with no `[functions]` `import_map` key, so the
import map is not pointed at; without the flag the `@supabase/supabase-js` bare
...
| `0 5 * * 0` | recipe-photo debris reap (Sunday; R-36b — schedule staged, function NOT yet deployed) | `recipe-photo-reap` |
...
bundle — there is no `supabase/seed/` directory. The committed seed migrations are
`20260520120000_r01_library_anon_seed.sql` (R-01 library anon owner + founding
seed rows), `20260523120100_f1_whole_foods_seed.sql` (F-1 whole-foods set,
generated by the `scripts/whole-foods/` pipeline),
`20260604120200_b1_catalog_seed.sql` (B1 free-exercise-db catalog — 873
system-row exercises), and `20260606120400_b2a_instructions_backfill.sql` (B2a
bilingual instructions backfill onto those catalog rows). They apply in order with
the rest of the migration history (`supabase start` / the `db-test` job from zero).
Ingredients are inserted as **system seeds** (`created_by_user_id = null`,
`source = 'system'`) so they are immutable and immediately visible to every user
via the shared library (see `data-model.md` Row-Level Security). A future BEDCA
seed of generic Spanish staples is an uncommitted product idea
...
- **Right to deletion:** the user-invoked `delete-account` edge function
  ...
  refined this (shipped 2026-05-20): the `delete-account` function calls the
  `reconcile_account_delete` RPC *before* `auth.admin.deleteUser`, reassigning
  the deleted user's shared-pool contributions (ingredients/recipes) to the
  reserved anon id `00000000-0000-0000-0000-00000000a0a0` rather than
...
20260523120000_f1_ingredients_name_en.sql           # applied 2026-05-23 (F-1): ingredients.name_en
...
**R-00 reproducibility check** (manual, not wired into CI): link the Supabase
CLI to prod and diff a from-zero `db reset` against the live schema —
`supabase db reset` then `supabase db diff --linked` (see the **Regen / verify
command** block above). There is no dedicated workflow or script for this on
