...
bundle — there is no `supabase/seed/` directory. The two seed migrations are
`20260520120000_r01_library_anon_seed.sql` (R-01 library anon owner + founding
seed rows) and `20260523120100_f1_whole_foods_seed.sql` (F-1 whole-foods set,
generated by the `scripts/whole-foods/` pipeline). They apply in order with the
rest of the migration history (`supabase start` / the `db-test` job from zero).
...
suite (`supabase/tests/00_schema`..`05_muscles.test.sql`) creates test users by
