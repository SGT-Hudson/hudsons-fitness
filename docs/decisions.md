...
## D-F28 — Tier-4 guards the select string, not the response shape

**Ruling:** The R-32 integration tier (`*.itest.ts`) validates that each fetch helper's PostgREST `.select(...)` string parses against the real schema, and nothing more. It invokes the 20 helpers that carry an explicit column/embed select with ids that match no row, and classifies the resulting PostgREST error against an **allow-list — only `PGRST116`** ("0 rows" from `.single()`) passes; `42703`/`PGRST200`/`PGRST100`/etc. fail the build. It asserts nothing about the returned data's shape. A coverage meta-test keeps the registry honest: it fails when a helper with an explicit select is unregistered or when a registry entry names a helper that no longer carries one.

**Why:** PostgREST validates the select before it applies filters, so a non-existent user id is enough to exercise the string — **no seed data and no auth session are needed**, which is what makes the tier cheap and stable. Asserting on response shape instead would require fixtures that age with every schema change and would duplicate the RLS/shape coverage Tier-3 pgTAP already owns. The accepted cost is that a wrong `as unknown as {…}` mapping in a helper still passes — that is a TypeScript-boundary concern, not a select-string one, and out of scope here. Bare `select('*')` / `select('*', { count })` and `select()` are excluded because they name no column and cannot break; mutations are excluded because they select `()`/`'*'`. On Node 20 the setup polyfills `WebSocket` from `ws` (test-only dep) and the tier fails closed against any non-local host, since `.env.test.local` carries the QA user's production credentials.

**Status:** decided · done (R-32 partial — the integration tier; the agent-browser e2e half stays open)
