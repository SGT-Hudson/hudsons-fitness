...
| `supabase/tests/database/03_rpc_behavior.sql` | Behavioral contracts: `materialize_plan_for_date` (future→0, no-week→0, idempotent, meal_type map), `save_recipe` create + ownership, `meal_log_one_source` CHECK, `phases` EXCLUDE non-overlap (`ON DELETE RESTRICT`/`mark_week_diverged` deferred; `handle_new_user` exercised indirectly in `01`) | Create |
...
> Note: `plan(23)` is the exact static assertion count: 1 RLS-all + 1 tdee_state RLS + 16 `policies_are` (one per table; the 15 baseline tables + `tdee_state`, with `ingredients` non-uniform) + 1 ingredients-role `is` + 1 `has_view` + 1 `col_is_unique` + 1 `hasnt_relation` + 1 `isnt_empty` = 23. pgTAP still fails loudly on any plan/observed mismatch, so a clean first run also re-confirms the count.
...
> The "exactly ONE DEFINER" assertion is the keystone drift guard for the D-C5/D-D6 invariant: if any future migration adds a DEFINER RPC, this fails. `prokind='f'` keeps it to plain/trigger functions (not procedures/aggregates); `handle_new_user` is excluded by name as the non-PostgREST trigger fn, so the expected count is exactly the lone sanctioned `apply_template_to_week_admin`. `plan(16)` is the exact static count (5 prosecdef INVOKER + 4 proconfig `search_path=""`/`=public` + 1 prosecdef DEFINER + 1 DEFINER-count + 2 admin-grant + 1 invoke_edge_function prosecdef + 1 invoke_edge_function grant = 16). Two serialization facts the assertions depend on: PostgreSQL stores `SET search_path TO ''` in `proconfig` as the **double-quoted** `search_path=""` (empty string is quoted), while a plain value is unquoted (`search_path=public`); and `information_schema.role_routine_grants` reports the PUBLIC pseudo-role as uppercase `PUBLIC` (named roles are lowercase) — both are baked into the expected literals so the guard is real, not false-green. pgTAP still self-checks the count on first run.
...
> mapping from the R-12 migration (index 1 → element 2 → 'lunch'). `plan(10)`
> is the exact static count (meal_log CHECK + 2 phases + 2 save_recipe + 5
> materialize contracts = 10); pgTAP still self-checks it on first run.
...
```
 ...
 pending a Docker+Supabase-CLI env — run `supabase start && supabase test
...
- [ ] **Deferred (needs Docker+CLI):** `supabase start && supabase test db`
