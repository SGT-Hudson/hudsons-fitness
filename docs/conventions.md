...
`decisions.md` (cited by D-id). A rule marked `⚠ Changing — see R-xx` is the
target; the codebase does not match it yet (tracked in `roadmap.md`).
...
- Derive the form type via `z.infer<typeof schema>`; no hand-written `type FormValues` (D-C3).

> ⚠ Changing — see R-09 (today forms are RHF without `zodResolver`; `@hookform/resolvers` not installed; migration pending).
...
- BMR (Mifflin-St Jeor) and target-weight are derived; recompute, never persist (D-B5). `> ⚠ Changing — see R-08` (4 dead `tdee_estimates` columns still present; `mifflinStJeor` not yet wired as display).
...
- Plan materialization is a single `SECURITY INVOKER` RPC (D-D6). `> ⚠ Changing — see R-12` (client/edge mirrors not yet removed).
- Convert the fat fraction (and any unit/fraction) only at the form boundary via a shared helper, never inline `×100` (D-B3). `> ⚠ Changing — see R-06` (helper not yet extracted; 3 inline sites).

## UI

- Use the shadcn `Badge` component for badges (D-D1). `> ⚠ Changing — see R-10` (Badge component not yet added; 4 inline-Tailwind sites).
...
- Bilingual ES/EN; for authenticated users `profile.language` is authoritative, applied at/after auth; pre-auth and fallback chain is `localStorage → navigator → es` (D-E1). `> ⚠ Changing — see R-13` (`profile.language` not yet applied at boot).
- Stored content (recipe/ingredient/template names) is never auto-translated — stays as authored (D-E2).
- Metric-only (kg/cm/g); no imperial (D-E3). `> ⚠ Changing — see R-14` (`profiles.units` legacy column still present, slated for removal).
- Authenticated language change is Settings-only; the one-click `LanguageSwitcher` appears only on pre-auth and onboarding routes (D-E4). `> ⚠ Changing — see R-15` (`LanguageSwitcher` still in `AppLayout` header).
