...
interface Props {
  /** Whole-recipe macros. */
  total: Macros;
  /** Per-serving macros — the highlighted column. */
  perServing: Macros;
  className?: string;
}
  ...
  return (
    ...
    >
      ...
      <div className="mb-1.5 mt-1 flex items-baseline gap-1">
        <span className="tnum text-[21px] font-semibold tracking-[-0.02em]">
      ...
      <div className="tnum grid grid-cols-3 gap-1 text-[10.5px]">
        {grams.map(({ key, value, dot }) => (
          <div key={key} className="flex flex-col">
            ...
            <b className="font-semibold">{Math.round(value)}g</b>
          </div>
        ))}
      </div>
    </div>
...
/**
 * The read view's macros card (canvas `RecetaMacrosCard` / `MRecetaMacros`):
 ...
 */
export function RecipeMacrosCard({ total, perServing, className }: Props) {
  const { t } = useTranslation('recetas');
  ...
  return (
    <Card className={cn('p-3.5 md:p-4', className)}>
      <h2 className="mb-2.5 text-[10.5px] font-medium uppercase tracking-[0.05em] text-muted-foreground">
        {t('detail.macrosTitle')}
      </h2>
      <div className="grid grid-cols-2 gap-2.5 md:gap-3.5">
        <MacroColumn title={t('macros.total')} macros={total} highlight={false} />
        <MacroColumn title={t('macros.perServing')} macros={perServing} highlight />
      </div>
